...
if (1 == Configuration::get('PS_TOKEN_ENABLE') AND
...
if ($context->customer->isLogged()) {
    ...
    for ($i = 1; false === empty($_POST['email' . strval($i)]); ++$i) {
        $to = Tools::getValue('email' . $i);
        $wishlist = WishList::exists($id_wishlist, $context->customer->id, true);
        if (false === $wishlist)
            exit($module->l('Invalid wishlist', 'sendwishlist'));
        if (false === WishList::addEmail($id_wishlist, $to))
            exit($module->l('Wishlist send error', 'sendwishlist'));
        $toName = strval(Configuration::get('PS_SHOP_NAME'));
        $customer = $context->customer;
        if (Validate::isLoadedObject($customer))
            Mail::Send(
                    $context->language->id, 'wishlist', sprintf(Mail::l('Message from %1$s %2$s', $context->language->id), $customer->lastname, $customer->firstname), array(
