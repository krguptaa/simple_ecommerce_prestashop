...
if ($context->customer->isLogged()) {
    if ($id_wishlist && true === WishList::exists($id_wishlist, $context->customer->id))
    ...
    if (($add || $delete) && false === empty($id_product)) {
        ...
        if ($add && $quantity) {
            ...
            echo (int) Db::getInstance()->getValue('SELECT count(id_wishlist_product) FROM '._DB_PREFIX_.'wishlist w, '._DB_PREFIX_.'wishlist_product wp where w.id_wishlist = wp.id_wishlist and w.id_customer='.(int) $context->customer->id);
            die();
        } else if ($delete)
