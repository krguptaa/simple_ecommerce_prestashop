...
class BlockWishListMyWishListModuleFrontController extends ModuleFrontController {
    ...
    public function assign() {
        ...
        if ($this->context->customer->isLogged()) {
            ...
            $add = (false === empty($add) ? 1 : 0);
            $delete = Tools::getIsset('deleted');
            $delete = (false === empty($delete) ? 1 : 0);
            $default = Tools::getIsset('default');
            $default = (false === empty($default) ? 1 : 0);
            $id_wishlist = Tools::getValue('id_wishlist');
            if (Tools::isSubmit('submitWishlist')) {
                if (1 == Configuration::get('PS_TOKEN_ACTIVATED') && strcmp(Tools::getToken(), Tools::getValue('token')))
                ...
                if (!count($errors)) {
                    ...
                    if (!count($errors)) {
                        ...
                        $wishlist->add();
                        Mail::Send(
                                $this->context->language->id, 'wishlink', Mail::l('Your wishlist\'s link', $this->context->language->id), array(
                            '{wishlist}' => $wishlist->name,
                            '{message}' => $this->context->link->getModuleLink('blockwishlist', 'view', array('token' => $wishlist->token)),
                                ), $this->context->customer->email, $this->context->customer->firstname . ' ' . $this->context->customer->lastname, null, strval(Configuration::get('PS_SHOP_NAME')), null, null, $this->module->getLocalPath() . 'mails/');
            ...
            elseif ($delete && false === empty($id_wishlist)) {
    ...
    /**
     * Ajax Process Product Change Wish list
     *
     * @return json
     */
